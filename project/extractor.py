...
# Устанавливается из main.py до начала обработки.
# Если None — fallback на системный temp (только при запуске extractor вне main.py).
PDF_TMP_DIR: Path | None = None
...
def _extract_pdf_text(path: str) -> str:
    ...
    if PDF_TMP_DIR is not None:
        tmp_path = str(PDF_TMP_DIR / f"docanalyzer_{uuid.uuid4().hex}.pdf")
    else:
        import tempfile
        tmp_path = str(Path(tempfile.gettempdir()) / f"docanalyzer_{uuid.uuid4().hex}.pdf")
